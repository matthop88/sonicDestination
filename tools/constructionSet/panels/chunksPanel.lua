...
return {
    create = function(self, stickyMouse, ids)
        ...
        local DATA   = require("tools/lib/dataStructures/lazyVal"):create()

        local chunkList = {}
        local WIDTH, HEIGHT = 256, 256
        for _, id in ipairs(ids) do table.insert(chunkList, CHUNK_TEMPLATE:create(id, CHUNKS, SOLIDS, DATA, WIDTH, HEIGHT)) end
        ...
        return {
            ...
            initChunkInfo = function(self, chunksName)
                ...
                self.CHUNKS:set(requireRelative("world/terrain/chunksBuilder"):create(CHUNKS_IMG, chunksName, CHUNKS_DATA))
                self.SOLIDS:set(requireRelative("world/terrain/solidsBuilder"):create(CHUNKS_DATA))
                self.DATA:set(CHUNKS_DATA)
            end,
